...
## Import regulatory motifs computed using CellOracle

```{r}
weightMatrix = read.csv(
  paste0("../celloracle/raw_GRN_for_",  tgtct, ".csv"),
  header = TRUE)
weightMatrix = weightMatrix[, c("source", "target", "coef_abs")]
weightMatrix = tidyr::pivot_wider(
  weightMatrix,
  names_from = target,
  values_from = coef_abs)
weightMatrix = as.data.frame(weightMatrix)
row.names(weightMatrix) = weightMatrix$source
weightMatrix = as.matrix(weightMatrix[, -1])
weightMatrix[is.na(weightMatrix)] = 0
rownames(weightMatrix) = paste0("Motif.", rownames(weightMatrix))
colnames(weightMatrix) = paste0("Gene.", colnames(weightMatrix))
```

## Import regulatory motifs computed using scenicplus

```{r}
eRegulon_AUC = read.csv(
  "../scenicplus/eRegulon_AUC_filtered.Gene_based.csv",
  header = FALSE)
eRegulon_names = as.character(eRegulon_AUC[1, -1])
eRegulon_AUC = eRegulon_AUC[-1, ]
eRegulon_AUC$V1 = sub("___cisTopic", "", eRegulon_AUC$V1)
eRegulon_AUC = eRegulon_AUC[
  eRegulon_AUC$V1 %in%
    proj6$cellNames[proj6$Clusters4BTmyelo == tgtct],
]
eRegulon_AUC = eRegulon_AUC[, -1]
eRegulon_AUC = matrix(as.numeric(as.matrix(eRegulon_AUC)),
                      ncol = ncol(eRegulon_AUC))
eRegulon_AUC = colMeans(eRegulon_AUC)

eRegulon_signatures = read.csv(
  "../scenicplus/eRegulon_signatures_filtered.Gene_based.csv",
  header = FALSE)
identical(eRegulon_names, eRegulon_signatures$V1)
eRegulon_signatures = eRegulon_signatures[, -1]
genes = sort(setdiff(as.character(as.matrix(eRegulon_signatures)), ""))

weightMatrix = matrix(
  0,
  nrow = length(eRegulon_names),
  ncol = length(genes))
for (i in 1:length(eRegulon_names)) {
  weightMatrix[i, ] = 1 * (genes %in% eRegulon_signatures[i, ])
}
weightMatrix = diag(eRegulon_AUC) %*% weightMatrix
# Atf2_+_(44g) Atf2_-_(10g)
rownames(weightMatrix) = paste0("Motif.", eRegulon_names)
colnames(weightMatrix) = paste0("Gene.", genes)

weightMatrix = weightMatrix[rowMaxs(weightMatrix) > 0, ]
weightMatrix = weightMatrix[, colMaxs(weightMatrix) > 0]

rm(eRegulon_AUC, eRegulon_names, eRegulon_signatures, genes)
```
...
plot(sort(log10(rowMeans(weightMatrix))))
x = quantile(rowMeans(weightMatrix), 0.85)
weightMatrix = weightMatrix[rowMeans(weightMatrix) > x, ] # arbitrary
...
# saveRDS(res, file = paste0("celloracle.NMF.rank", r, ".", tgtct, ".rds"))
# saveRDS(res, file = paste0("scenicplus.NMF.rank", r, ".", tgtct, ".rds"))
...
# saveRDS(result, file = paste0("celloracle.NMF.rank", r, ".", tgtct, ".GSEA.rds"))
# saveRDS(result, file = paste0("scenicplus.NMF.rank", r, ".", tgtct, ".GSEA.rds"))
...
dbnamegsname = rbind(
  ...
  data.frame(dbname = 'REACTOME', gsname = 'REACTOME_INNATE_IMMUNE_SYSTEM%%M1036'), # scenicplus
  ...
  data.frame(dbname = 'GOBP', gsname = 'GOBP_ADHERENS_JUNCTION_MAINTENANCE%%M23262') # scenicplus
)
```
