...
lapply(
  1:dim(res)[3],
  function (i) {
    x = sXgslist[[i]]$u[1:nrow(XGene), 1] * sXgslist[[i]]$d[1] / sqrt(ncol(XGene) - 1)
    ...
    x = sXgslist[[i]]$u[nrow(XGene) + 1:nrow(XMotif), 1] * sXgslist[[i]]$d[1] / sqrt(ncol(XMotif) - 1)
    ...
    p1 =
      ggplot(data = dataplot,
             aes(x = order,
                 y = score)) +
      geom_col() +
      geom_text(aes(label = name, y = -0.05 * sign(score), hjust = -0.5 * sign(score) + 0.5), angle = -90) +
      theme(
...
In the code block below, check if the genes in `gs` are co-regulated, more than "average".
If so, we analyze differential expression in the direction of `gs`,
which might or might not be differentially expressed between diet conditions.

Use `matnrDEGene` and `matnrDEMotif`, which should include less noise.
When `gs` is not differentially expressed between diet conditions,
...
a0 = lm(sXgs$v[, 1] * sqrt(nrow(sXgs$v)) * sXgssign ~ 0 + A)
...
```{r}
dataplot = data.frame(
  score = rbind(XGene, XMotif) %*% sXgs$v[, 1] / sqrt(ncol(XGene) - 1) * sXgssign,
...
p1 =
  ...
  labs(x = "Correlation with GS expression",
