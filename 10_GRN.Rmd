...
```{r}
dataplot = data.frame(
  score = rbind(XGene, XMotif) %*% sXgs$v[, 1] * sXgssign,
...
p1 =
  ...
  geom_text(data = dataplot[abs(dataplot$score) > 30 & dataplot$interact == "High", ],
            aes(x = score,
                y = ppinteractionwithgs,
                label = name),
            vjust = 0,
...
Differential motif expression against `gslevel`

```{r}
identical(colnames(matnrMotif), names(gslevel))
resultcompared = broom::tidy(
  lm(y ~ x,
     data = list(
       y = t(assay(matnrMotif)),
       x = gslevel)))
resultcompared = resultcompared[resultcompared$term == "x", ]
colnames(resultcompared)[colnames(resultcompared) == "p.value"] = "p"
resultcompared$FDR = p.adjust(resultcompared$p, method = "BH")
