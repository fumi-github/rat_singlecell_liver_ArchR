...
GENIE3 and NMF quantify the importance of genes/motifs as variance.
...
Check if the weighted genes are co-regulated more than randomly chosen genes (ie more than "average").
The permutation of wGene, wMotif is an interesting procedure,
however the corresponding null hypothesis does not seem to be justifiable.
  ...
  function (i) { # factor
    ...
    sapply(
      ...
      function (j) { # permutation trial
        x = irlba::irlba(rbind(XGene * wGene[sample(nrow(wGene)), i],
                               XMotif * wMotif[sample(nrow(wMotif)), i]))
        cbind(
          data.frame(
            d1 = x$d[1],
            maxabsu1Gene = max(abs(x$u[1:nrow(wGene), 1])),
            maxabsu1Motif = max(abs(x$u[nrow(wGene) + 1:nrow(wMotif), 1])),
            maxabsscore1Gene = max(abs(x$u[1:nrow(wGene), 1] * x$d[1])),
            maxabsscore1Motif = max(abs(x$u[nrow(wGene) + 1:nrow(wMotif), 1] * x$d[1]))),
          as.data.frame(as.list(abs(tapply(x$v[, 1], A, mean)))))
      })
  },
  simplify = "array")
...
lapply(
  1:dim(res)[3],
  function (i) {
    mean(as.numeric(sXgsnull["d1", , i]) > sXgslist[[i]]$d[1])
  })
```
  ...
  function (i) {
    ...
    fwe =
      sapply(x, function (xi) {
        mean(abs(xi) < as.numeric(sXgsnull["maxabsscore1Gene", , i])) })
    x = x * sXgssign[i]
    x = c(head(sort(x), 5), tail(sort(x), 5))
    fwe = fwe[names(x)]
    dataplot = data.frame(
      order = 1:length(x),
      score = x,
    ...
    fwe =
      sapply(x, function (xi) {
        mean(abs(xi) < as.numeric(sXgsnull["maxabsscore1Motif", , i])) })
    x = x * sXgssign[i]
    x = c(head(sort(x), 5), tail(sort(x), 5))
    fwe = fwe[names(x)]
    dataplot = rbind(
...
dataplot =
  do.call(
    ...
    lapply(
      1:dim(res)[3],
      function (i) {
        x = tapply(sXgslist[[i]]$v[, 1], A, mean)
        P = rowMeans(sXgsnull[levels(A), , i] > abs(as.numeric(x)))
        data.frame(
          diet = names(x),
          level = x * sqrt(nrow(sXgslist[[i]]$v)) * sXgssign[i],
          P = P,
          Module = paste0("Module ", which(moduleindex == i)))
      }))
p1 =
